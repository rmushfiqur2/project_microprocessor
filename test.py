...
import numpy as np
host_address = "http://192.168.0.8:8080/"
...
from train_extractImages import extract_files
from train_mlp import main_mlp
...
class Thread(QThread):
	...
	def myCamFun(self):
		...
		area = [box[2] * box[3] for box in faces]
		max_area = max(area)
		...
		if matching:
			with graph.as_default():
				...
				feature = model.extract('tem_face.jpg')
...
class Ui_MainWindow(object):
	...
	def retranslateUi(self, MainWindow):
		...
		self.label_2.setText(_translate("MainWindow", "Valo asi, valo theko\n"))
	...
	def faceDetected(self, int_code):
		...
		zipped = zip(users, allowed)
		zipped = sorted(zipped)
		users = [u for u, a in zipped]
		allowed = [a for u, a in zipped]
		if int_code < len(users):
			user = users[int_code]
			open_door = allowed[int_code]
			engine = pyttsx3.init()
			if int(open_door)>0.5:
				...
				#winsound.PlaySound('gunshot.wav', winsound.SND_ALIAS | winsound.SND_ASYNC)
